...
def startPeer():
    ...
    nick = raw_input("Ola, qual e o seu nome? ")  # Nick do peer
    host = raw_input('Qual e o seu ip? ')  # Ip do peer
    port = int(raw_input('Qual e a sua porta? '))  # Porta do peer
    ...
    while cmd != '3':
        cmd = raw_input(
            ...
            "uma sala\n3 - Para sair\n")
        if cmd == '1':  # Criar sala
            roomName = raw_input('Qual sera o nome da sala? ')
            ...
            print('Sua sala foi criada no endereco ' + str(host) + ' e porta {port}!!')
            time.sleep(0.03)
            ...
            roomIP = raw_input('Qual e o IP da sala? ')  # Ip do adm da sala
            roomPort = int(raw_input('Qual e a porta da sala? '))  # Porta do adm da sala
            if (roomIP, roomPort) in roomsBanned:
                ...
                try:  # Se o usuario inserir um endereco de sala (ip, porta) inexistente, um erro e gerado no socket, logo, o try e necessario
                    ...
                    answer = splitMessage(str(socket_.recv(1024)))  # Resposta do servidor, se foi aceito ou nao na sala
